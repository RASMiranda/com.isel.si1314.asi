...
echo Running 1.03.6.CreatePublication-PeerToPeer-Reclamacao-LHD.sql na lojas de produtos de crianca e senhora.
sqlcmd -b -H localhost -S "%LSC%" -i 1.03.6.CreatePublication-PeerToPeer-Reclamacao-LHD.sql -v SEDE="%SEDE%" ESTA_LOJA="%LSC%" LOJA_REMOTA="%LHD%"
if not %errorlevel% == 0 goto end

echo Running 1.03.6.CreatePublication-PeerToPeer-Reclamacao-LSC.sql na loja de produtos para homem e desportistas.
sqlcmd -b -H localhost -S "%LHD%" -i 1.03.6.CreatePublication-PeerToPeer-Reclamacao-LSC.sql -v SEDE="%SEDE%" ESTA_LOJA="%LHD%" LOJA_REMOTA="%LSC%"
if not %errorlevel% == 0 goto end
